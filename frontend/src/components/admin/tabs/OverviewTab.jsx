  ...
  return (
    <div className="space-y-6 animate-rise">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold font-display">Dashboard Overview</h3>
        <button onClick={load} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary cursor-pointer">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        {[
          ...
          <div key={card.label} className="surface-card p-5">
            ...
            <p className="mt-2 text-2xl font-bold font-display">{card.val}</p>
          </div>
        ))}
      </div>

      {overview?.unreadMessages > 0 && (
        <div className="rounded-xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-xs text-destructive font-semibold">
          📬 {overview.unreadMessages} unread message{overview.unreadMessages > 1 ? "s" : ""} in inbox
        </div>
      )}

      {overview?.profile && (
        <div className="surface-card p-6">
          <h3 className="text-base font-bold font-display">Active Profile (from DB)</h3>
